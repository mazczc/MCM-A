...
s = species(1.0, 0.01, 0.20, 0.002, 0.5, 2.0, 0.1)
species_dic["Xiao Qiang"] = s

s = species(2.0, 0.1, 0.4, 0.004, 0.85, 2.0, 0.5)
species_dic["Zhang"] = s
...
DStart = 1000
DEnd = 1050
I = [np.random.normal(1.5, 0.1) for i in range(Time)]
...
Recover = (np.sum(BlistNormal[DEnd + 50]) - np.sum(BlistDrought[DEnd + 50])) /np.sum(BlistNormal[DEnd + 100])
